...
func SaveConfigurationToFile(technology string, ecrEndpoint string, ctx string, namespace string, useDefaultKubeConfig bool, kubeConfigPath string, dirName string, registry string, command string, port int, dbConfig config.DatabaseConfig, installationCommands []string, dir string) {
	build.SaveConfigToFile(technology, ecrEndpoint, ctx, namespace, useDefaultKubeConfig, kubeConfigPath, dirName, registry, command, port, dbConfig, installationCommands, dir)
}
